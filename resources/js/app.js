...
var netSavings = [];
for (
  var i = collect.timeline.length >= 7 ? collect.timeline.length - 7 : 0;
...
document.getElementById("inpAmount").addEventListener("keydown", function (e) {
  if (e.keyCode === 13) {
    var type = document.getElementById("toggleType").checked
      ? "withdraw"
      : "deposit";
    if (this.value) newFeed(Number(this.value), type);
...
document.getElementById("toggleType").addEventListener("change", function () {
  ...
  if (this.checked) {
    // Withdraw Action
    parent.children[2].style.color = "#e45c5c";
    button.style.backgroundColor = "#e45c5c";
...
function newFeed(amount, type) {
  if (type == "withdraw" && savings < amount) {
    alert("You don't have enough balance to make this withdrawal!");
    return;
  }
...
function closeFeedForm() {
  ...
  if (button.style.backgroundColor != "rgb(137, 222, 141)") {
    // Reset the form from withdraw to deposit action
    button.style.backgroundColor = "#89de8d";
